\chapter{Power Budget}
 ...
 A CubeSat will have standard set of satellite subsystems: Structural subsystem, Telemetry, Electrical Power Subsystem (EPS), thermal subsystem (TCS), Attitude Determination and Control Subsystem (ADCS, On Board Combuter (OBC) and Payload. For the calculations, we selected payload as a LoRa module which is the payload of GECB satellite program (cite reference). The orbital parameters are given below:
\begin{table}[]
	\begin{tabular}{|l|l|}
		\hline
	...
	\end{tabular}
\end{table}
 The CubeSat has an orbital altitude of 590 km with an orbital radius of 6968.14 km to maintain a flight velocity of 7.563km km/s. The orbital period is 96 min 29 sec with an eclipse time of 31 min 10 sec and daylight time of 65 min 19 sec. Based on the power budget, the energy required by the CubeSat is 1.997 Wh per orbit. Hence, the solar panels must designed be able to produce at least 1.997 Wh per orbit.
 \\
 
 Power requirements of various components of each subsystems are given below. Since the power production may vary due to parameters like efficiency of panels, margin and contingency are added to the total power requirements(cite reference).\\
 \begin{table}[]
